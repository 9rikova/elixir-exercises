defmodule SudokuSolver do
  ...
  def solve(sudoku, format, method) when is_list(sudoku),
    do: solve(Matrix.from_list(sudoku), format, method)
    ...
    case find_empty_field(sudoku) do
      ...
      {row, col} ->
        case find_possible_numbers(sudoku, row, col) do
          ...
          possible_numbers ->
            Task.async_stream(possible_numbers, fn n ->
              async_solve(put_in(sudoku[row][col], n))
            end)
  ...
  defp possible_number?(sudoku, row_coord, col_coord, n) do
    ...
    square_valid =
      Enum.all?(square_row_start..(square_row_start + 2), fn row ->
  ...
  defp validate_sudoku(sudoku) do
    valid_row_size =
      Enum.all?(Map.values(sudoku), fn row -> is_map(row) && Enum.count(Map.keys(row)) == 9 end)
    ...
    valid_values =
      Enum.all?(@coords, fn row ->
        Enum.all?(@coords, fn col ->
          ...
          num in 0..9 &&
            (num == 0 || possible_number?(put_in(sudoku[row][col], 0), row, col, num))
        end)
      end)
